...
const client =
  window.location.protocol === 'https:'
    ? new Nes.Client(`wss://${window.location.host}`)
    : new Nes.Client(`ws://${window.location.host}`);
...
function connectClient() {
  if (!connectionPromise) {
    ...
    connectionPromise = connectWithAuth().catch(() => {
...
export { subscribe, subscribeError, requestInitialData, onConnectionError };
