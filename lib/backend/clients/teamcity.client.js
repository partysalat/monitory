...
module.exports.create = ({ url }) => {
  function mapJobs(jobs) {
    return jobs.data.buildType
      .filter(
        (build) =>
          build.builds.build[0] && build.builds.build[0].status !== 'SUCCESS'
      )
      .map((build) => ({
        name: `${build.projectName} / ${build.name}`,
        assignee: _.get(build, 'investigations.investigation[0].assignee.name'),
      }));
  }
  ...
  function getFailedBuildsFor(projects = []) {
    return Promise.all(projects.map(getFailedBuilds)).then((result) =>
      _.flatten(result)
    );
  }
  return {
