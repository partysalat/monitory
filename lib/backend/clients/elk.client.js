...
module.exports.create = ({ url, alias }) => {
  ...
  const queryEs = (composedUrl, data) =>
    axios({
      method: 'POST',
      url: composedUrl,
      headers: {
  ...
  const getCountFor = (query, timeSpan = 'now-1h') => {
    ...
    return queryEs(composedUrl, getEsResultQueryBaseOn(query, timeSpan)).then(
      ({ data }) => {
  ...
  const getHistogramFor = (query, timeSpan = 'now-1h', interval = '30s') => {
    ...
    return queryEs(
      composedUrl,
      getEsFacetQueryBaseOn(query, timeSpan, interval)
    )
      .then(({ data }) => {
