...
function compileAndWriteMainFiles(dashboardGlob, jsAssetsDir) {
  const mainTemplateRaw = fs.readFileSync(
    path.resolve(__dirname, '..', '..', 'frontend', 'main.template.jsx'),
    'utf-8'
  );
  ...
  const mainFiles = files.map(([fileName, pathToDashboard]) => [
    fileName,
    mainTemplateRaw
  ...
  mainFiles.forEach(([fileName, content]) =>
    fs.writeFileSync(path.resolve(jsAssetsDir, fileName), content)
  );
  return files;
}
module.exports.compileAssets = async (dashboardGlob, jsAssetsDir) => {
  ...
  return isDebugMode()
    ? buildContext.watch()
    : buildContext.rebuild().then((data) => {
        logger.info(
          `Compiling assets finished! Took ${
