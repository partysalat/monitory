...
describe('assets server', () => {
  ...
  describe('when calling compileAssetsAndStartServer', () => {
    let webpackConfig;
    beforeEach(() => {
      cut = require('./assets');
      cut.compileAssetsAndStartServer(ANY_DASHBOARD_GLOB, ANY_ASSETS_SERVER_PORT);
      webpackConfig = serveMock.mock.calls[0][1].config;
    });
    it('calls glob.sync with absolute option', () => {
      expect(globMock.sync).toHaveBeenCalledWith(ANY_DASHBOARD_GLOB, { absolute: true });
    });
    it('sets the server assets port', () => {
      expect(webpackConfig.serve.port).toEqual(ANY_ASSETS_SERVER_PORT);
    });
    it('sets the entries', () => {
  ...
  function mockWebpackServe() {
    jest.mock('webpack-serve');
    serveMock = require('webpack-serve');
  }
});
